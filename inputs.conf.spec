...
* How an HTTP Event Collector (HEC) input enforces load balancer cookie markers
  for HEC indexer acknowledgment (ACK) endpoint requests.
...
* Specifies the cookie markers to search for in the "Cookie" request header
  when validating Acknowledgement (ACK) endpoint requests.
* The input matches values as case-insensitive substrings anywhere in the 
  "Cookie" request header.
* If this setting has no value, the Splunk platform performs no
  enforcement, which is equivalent to the behavior when the
  'headerEnforcementMode' setting has a value of "off".
* Example: AWSELB,AWSALB,GCLB,GCILB,ApplicationGatewayAffinity,ARRAffinity,
  AGCAffinity,ASLBSA
* Default: AWSELB,AWSALB,GCLB,GCILB,ApplicationGatewayAffinity,ARRAffinity,
  AGCAffinity,ASLBSA
...
preventUiFromDoingSophisticatedHttpEventCollectorStateInference = <boolean>
* Currently not supported. This setting is related to a feature that is
  still under development.
* Whether or not Splunk Web layers overall HTTP Event Collector input state
  with the enabled state of individual HEC tokens, as implemented in the
  web_v2/search_mrsparkle/exposed/js/views/inputs/http/GridRow.js file.
* If set to "true", Splunk Web does not layer overall HTTP Event Collector input
  state with the enabled/disabled state of individual tokens.
* Default: false
...
remote_queue.type = [sqs|kinesis|sqs_smartbus|sqs_smartbus_cp|sqs_datalake|asq|gcs_smartbus]
* Required.
...
remote_queue.sqs_datalake.index_raw_tokens = <boolean>
* Currently not supported. This setting is related to a feature that is still 
  in development.
* Controls whether events are searchable by raw terms.
* A value of "true" means that events are searchable by raw terms.
* A value of "false" means that events are not searchable by raw terms.
  * In this case, search predicates must be of the form 'key=value' to match 
    the ingested events.
* This setting is optional.
* Default: true
